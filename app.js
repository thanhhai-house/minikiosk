...
function uniqueTypes(arr) {
  return [...new Set(arr.map((x) => x.type).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b)
  );
}
function uniqueBrands(arr) {
  return [...new Set(arr.map((x) => x.brand).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b)
  );
}
...
// ===== Form reset (✅ thiếu cái này là crash) =====
function clearForm() {
  elId.value = "";
...
function render() {
  ...
  elList.innerHTML =
    ...
    `;
      ...
      .join("") || `<div class="muted">Chưa có sản phẩm. Hãy thêm 1 món để thấy danh sách.</div>`;
}
...
window.openDetail = function (id) {
  ...
  dMoves.innerHTML = moves.length
    ...
    `;
        ...
        .join("")
    : `<div class="muted">Chưa có lịch sử.</div>`;
...
btnExportInv.addEventListener("click", () => {
  ...
  const rows = [header].concat(
    items.map((it) => [
      it.id, it.oem, it.name, it.brand, it.type, it.price, it.qty,
      fmtDate(it.createdAt), fmtDate(it.updatedAt)
    ])
  );
